...
var calPoints = function (operations) {
  ...
  for (let i = 0; i < operations.length; i++) {
    ...
    } else if (isNaN(operations[i]) && operations[i] === "C") {
      records.pop();
    } else if (isNaN(operations[i]) && operations[i] === "D") {
      const addDouble = records.at(-1) * 2;
      records.push(addDouble);
    } else if (operations[i] === "+") {
      const sumOfPrev = records.at(-1) + records.at(-2);
      records.push(sumOfPrev);
    }
  }
